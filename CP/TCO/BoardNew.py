...
class Sensor(object):
    ...
    def __init__(self, data, resolution):
        ...
        df = pd.DataFrame({'A':[25, 55, 80, -20, -40, 25]})
        #print(df)
        self.data['Temperature (C)'] = df.values
        print(self.data)
...
#        self.calculate_tcs()
    ...
    def calculate_tco(self):
        ...
        for i in self.data.index.unique():
            ...
            self.calculate_1g_offset(self.data[self.data.index == i])
...
if __name__ == "__main__":
    ...
    for tin in tins:
        ...
        if tin is not tins[0]:
            tco_curr = TCO(adj_data)
            print(tco_curr.char) 
            for i in tco_curr.char.columns.values:
                tco_curr.char.loc['Average', i] = average([tco_curr.char.loc['Average',i] , tco_last.char.loc['Average',i]])
                
                tco_curr.char.loc['St. Deviation',i] = average([tco_curr.char.loc['St. Deviation',i] , tco_last.char.loc['St. Deviation',i]])
                
                tco_curr.char.loc['Maximum',i] = max([tco_curr.char.loc['Maximum',i] , tco_last.char.loc['Maximum',i]])
                
                tco_curr.char.loc['Minimum',i] = min([tco_curr.char.loc['Minimum',i] , tco_last.char.loc['Minimum',i]])
                
                tco_curr.char.loc['Abs. Average',i] = average([tco_curr.char.loc['Abs. Average',i] , tco_last.char.loc['Abs. Average',i]])
        else:
            tco_curr = TCO(adj_data)
            print(tco_curr.char) 
        tco_last = tco_curr
    ...
    outdir = "C:/Users/"+getpass.getuser()+"/Desktop/PythonAutomatedProcessing/tco/"+str(tins)
    if not os.path.exists(outdir):
        os.makedirs(outdir)  
    
    tco.char.to_csv(outdir+"/tco"+str(tins)+".csv")
    tco.summary.to_csv(outdir+"/tco_sum"+str(tins)+".csv")
    
    tco.hist_tco('x')
    plt.savefig(outdir+"/x_tco"+str(tins)+".png", bbox_inches='tight')
    
    tco.hist_tco('y')
    plt.savefig(outdir+"/y_tco"+str(tins)+".png", bbox_inches='tight')
    
    tco.hist_tco('z')
    plt.savefig(outdir+"/z_tco"+str(tins)+".png", bbox_inches='tight')
    
    tco.plot_offset('x')
    plt.savefig(outdir+"/x_offset"+str(tins)+".png", bbox_inches='tight')
    
    tco.plot_offset('y')
    plt.savefig(outdir+"/y_offset"+str(tins)+".png", bbox_inches='tight')
    
    tco.plot_offset('z')
    plt.savefig(outdir+"/z_offset"+str(tins)+".png", bbox_inches='tight')
