...
def hist_SleepIDD(data, outdir):
    ...
    hist, bins = np.histogram(data["Sleep IDD (uA)".upper()], bins=x_min_ticks)
    ...
    plt.savefig(outdir+"/Sleep Current_"+dt+".png", bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    read = pd.read_excel(root.filename, usecols = ['BIN_No','300:Idd_sleep_1.65V', '301:Idd_sleep_2.8V', '302:Idd_sleep_3.6V'])
    #drop anything that isn't in bin 1, and then remove the bin column
    read = read[read.BIN_No == 1].drop(['BIN_No'], axis=1)
    #drop null values and non numeric values
    #read = read.dropna()
    read = read[~read.isin(['A'])]
    #rename headers for graphs
    df1 = read['300:Idd_sleep_1.65V']
    df1.rename(columns={"300:Idd_sleep_1.65V":"Sleep IDD (uA)"})
    df2 = read['301:Idd_sleep_2.8V']
    df2.rename(columns={"301:Idd_sleep_2.8V":"Sleep IDD (uA)"})
    df3 = read['302:Idd_sleep_3.6V']
    df3.rename(columns={"302:Idd_sleep_3.6V":"Sleep IDD (uA)"})
    Data= pd.DataFrame()
    Data.columns = ['Sleep IDD (uA)']
    Data = pd.concat([df1,df2,df3])*1000000
    Data = Data.dropna()   
    print(Data)
    
    outdir = "C:/Users/"+getpass.getuser()+"/Desktop/PythonAutomatedProcessing/FT/Sleep Current"
