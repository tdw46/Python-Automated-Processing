...
def dataSummary(data):
    ...
    for i in p:
        # char.set_value("Average", i, data[i].mean())
        # char.set_value("St. Deviation", i, data[i].std())
        # char.set_value("Maximum", i, data[i].max())
        # char.set_value("Minimum", i, data[i].min())
        # char.set_value("Abs. Average", i, data[i].abs().mean())
...
if __name__ == "__main__":
    ...
    read = pd.read_excel(root.filename, usecols = ['BIN_No','7303:Rotation Check - X : 0g', '7306:Rotation Check - Y : 0g', '7309:Rotation Check - Z : 0g'])
    ...
    Data = Data*3.906 #*1000/256, conversion to get mg output
    print(Data)
